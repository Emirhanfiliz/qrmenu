...
export default function ProductsPage() {
  ...
  const openEdit = (p: Product) => {
    setEditing(p);
    setForm({ categoryId: p.category.id, name: p.name, description: p.description ?? '', price: p.price, imageUrl: p.imageUrl ?? '', isAvailable: p.isAvailable });
    setShowForm(true);
  };
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={openNew} className="px-4 py-2 bg-gold hover:bg-gold-dim text-void font-display font-semibold text-sm rounded-lg transition-colors">Yeni Urun</button>
      </div>

      {/* Category filter */}
      {categories.length > 0 && (
        <div className="flex gap-2 flex-wrap mb-6">
          <button onClick={() => setFilterCat('')} className={`px-3 py-1.5 rounded-full text-xs font-body transition-all ${!filterCat ? 'bg-gold/15 text-gold' : 'bg-surface border border-border text-silver hover:text-snow'}`}>Tumu</button>
          {categories.map((c) => (
            <button key={c.id} onClick={() => setFilterCat(c.id)} className={`px-3 py-1.5 rounded-full text-xs font-body transition-all ${filterCat === c.id ? 'bg-gold/15 text-gold' : 'bg-surface border border-border text-silver hover:text-snow'}`}>{c.name}</button>
          ))}
        </div>
      )}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-surface border border-border rounded-2xl p-7 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h2 className="font-display text-snow text-lg font-semibold mb-6">{editing ? 'Urun Duzenle' : 'Yeni Urun'}</h2>
            <form onSubmit={submit} className="flex flex-col gap-4">
              ...
              <div className="flex gap-3 mt-2">
                ...
                <button type="submit" disabled={loading} className="flex-1 py-2.5 bg-gold hover:bg-gold-dim text-void font-display font-semibold text-sm rounded-lg transition-colors disabled:opacity-50">{loading ? 'Kaydediliyor...' : 'Kaydet'}</button>
              </div>
            </form>
          </div>
        </div>
          ...
          {filtered.map((p) => (
            ...
            >
              ...
              <div className="flex gap-2 flex-shrink-0">
                ...
                <button onClick={() => remove(p.id)} className="px-3 py-1.5 text-xs font-body text-red-400 hover:text-red-300 border border-red-900/30 hover:border-red-400/30 rounded-lg transition-colors">Sil</button>
              </div>
            </div>
          ))}
        </div>
