...
describe('AdminService', () => {
  ...
  describe('approveAndSubscribe', () => {
    it('throws NotFoundException when restaurant not found', async () => {
      ...
      await expect(
        service.approveAndSubscribe('bad-id', SubscriptionType.TRIAL)
      ).rejects.toThrow(NotFoundException);
    });

    it('runs transaction to activate restaurant and create subscription', async () => {
      mockPrisma.restaurant.findUnique.mockResolvedValue({ id: 'r1', subscription: null });
      mockPrisma.$transaction.mockResolvedValue([]);

      await service.approveAndSubscribe('r1', SubscriptionType.TRIAL);
  ...
  describe('renewSubscription', () => {
    it('throws NotFoundException when restaurant not found', async () => {
      mockPrisma.restaurant.findUnique.mockResolvedValue(null);
      await expect(
        service.renewSubscription('bad-id', SubscriptionType.ANNUAL)
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('creates subscription when none exists', async () => {
      ...
      await service.renewSubscription('r1', SubscriptionType.ANNUAL);
    ...
    it('updates existing subscription', async () => {
      mockPrisma.restaurant.findUnique.mockResolvedValue({
        id: 'r1',
        subscription: { restaurantId: 'r1' },
      });
      mockPrisma.subscription.update.mockResolvedValue({});

      await service.renewSubscription('r1', SubscriptionType.TRIAL);
  ...
  describe('suspendRestaurant', () => {
    it('throws NotFoundException when restaurant not found', async () => {
      mockPrisma.restaurant.findUnique.mockResolvedValue(null);
      await expect(service.suspendRestaurant('bad-id')).rejects.toThrow(NotFoundException);
    });
    ...
    it('sets restaurant status to SUSPENDED', async () => {
      ...
      await service.suspendRestaurant('r1');
